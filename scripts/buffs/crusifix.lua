...
local function postPickupInit(_, pickup)
    if Game():GetRoom():GetType() == RoomType.ROOM_BOSS and Resouled:BuffPresent(Resouled.Buffs.CRUSIFIX) then
...
local function prePickupCollision(_, pickup, collider)
    ...
    if player then
        local ROOM_SAVE = SAVE_MANAGER.GetRoomSave(pickup)
        if ROOM_SAVE and ROOM_SAVE.Resouled_CrusifixItemID then
            if pickup.SubType == ROOM_SAVE.Resouled_CrusifixItemID then
                if player:GetMaxHearts() > 2 then
                    player:AddMaxHearts(-2, true)
                else
                    ROOM_SAVE.Resouled_CrusifixItemID = nil
                end
            end
        end
    end
