...
Resouled.Curses = {
    ...
    CURSE_OF_AMNESIA = Isaac.GetCurseIdByName("Curse of Amnesia!"),
}
...
Resouled.CursesMapId = {
    ...
    [Resouled.Curses.CURSE_OF_AMNESIA] = "ResouledCurseOfAmnesia"
}
...
include("scripts.curses.curse_of_loss")
include("scripts.curses.curse_of_blood_lust")
include("scripts.curses.curse_of_soulless")
include("scripts.curses.curse_of_empathy")
include("scripts.curses.curse_of_fatigue")
include("scripts.curses.curse_of_envy")
include("scripts.curses.curse_of_the_hollow")
include("scripts.curses.curse_of_the_suspicious")
include("scripts.curses.curse_of_tiny_hands")
include("scripts.curses.curse_of_the_hunted")
include("scripts.curses.curse_of_impulse")
include("scripts.curses.curse_of_amnesia")
...
local function onCurseEval(_, curses)
    ...
    if curses ~= 0 and Resouled:GetCursesNum(curses) == 1 then -- CHALLNGE CURSES ARE APPLIED AFTER THIS SO NO NEED TO CHECK HOW MANY CURSES ARE ALREADY PRESENT BUT I ADDED IT JUST SO IT DOESNT MESS WITH OTHER MODS
        ...
        if curses == LevelCurse.CURSE_OF_LABYRINTH and (stage > 7 or stage%2 == 0) then
            Resouled:NewSeed()
            goto RollCurse
        end
