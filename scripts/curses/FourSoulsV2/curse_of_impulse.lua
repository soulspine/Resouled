...
local function onPlayerCollision(_, player, gridIndex, gridEntity)
    ...
    if Resouled:CustomCursePresent(curse) and gridEntity and not data.ResouledCurseOfImpulsePushCooldown then
        player:AddKnockback(EntityRef(player), Resouled:GetReflectionVector(player.Velocity, Resouled:SnapDirectionToAxis(player.Velocity)) * PUSH_VELOCITY_MULT, PUSH_DURATION, false)
