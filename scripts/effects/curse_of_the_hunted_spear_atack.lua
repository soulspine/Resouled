...
local function postEffectUpdate(_, effect)
    ...
    if sprite:IsPlaying("Idle") then
        ...
        effect.Velocity = Vector(1, 0):Normalized():Rotated(effect.SpriteRotation) * SPEAR_SPEED
        effect.Position = effect.Position + effect.Velocity

        if sprite:IsEventTriggered("ResouledTrail") then
            local trail = Game():Spawn(COTH_TYPE, COTH_VARIANT, effect.Position, Vector.Zero, effect, 0, effect.InitSeed)
