...
local function onCacheEval(_, player)
    player:CheckFamiliar(FAMILIAR_VARIANT, player:GetCollectibleNum(ULTRA_FLESH_KID), player:GetCollectibleRNG(ULTRA_FLESH_KID), nil, FAMILIAR_SUBTYPE)
end
...
local function onFamiliarUpdate(_, familiar)
    if familiar.SubType == FAMILIAR_SUBTYPE then
        ...
        if not familiar.Target then
            ...
            local target = Resouled.FamiliarTargeting:SelectNearestEnemyTarget(familiar)
