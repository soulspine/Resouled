...
local function postNpcDeath(_, npc)
    if Resouled:RoomEventPresent(Resouled.RoomEvents.BLESSING_OF_GREED) then
        local coin = Resouled.Game:Spawn(EntityType.ENTITY_PICKUP, PickupVariant.PICKUP_COIN, npc.Position, Vector(1 ,0):Rotated(math.random(0, 360) * math.random(MIN_VELOCITY, MAX_VELOCITY)), nil, CoinSubType.COIN_PENNY, npc.InitSeed)
